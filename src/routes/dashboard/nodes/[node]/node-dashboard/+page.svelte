  ...
  <div class="nd-grid">
    ...
    <div class="nd-left">
      ...
      <div class="status-card">
        <div class="status-banner" class:banner-online={isOnline} class:banner-offline={!isOnline}>
          <span class="status-chip" class:chip-online={isOnline} class:chip-offline={!isOnline}>
            <span class="status-dot" class:dot-online={isOnline} class:dot-offline={!isOnline}></span>
        ...
        <div class="info-section">
          <div class="info-row">
            ...
            <span class="info-badge" class:badge-ok={serviceStatus?.emittable || nodeInfo?.emittable} class:badge-fail={!(serviceStatus?.emittable || nodeInfo?.emittable)}>
              {(serviceStatus?.emittable || nodeInfo?.emittable) ? 'True' : 'False'}
            </span>
          </div>
          <div class="info-row">
            <span class="info-label">Emit Count</span>
            <span class="info-value">{(nodeInfo?.emit_count ?? 0).toLocaleString()}</span>
          </div>
        </div>
        ...
        {#if diskData}
          <div class="disk-section">
            ...
            <div class="progress-track">
              <div class="progress-fill" style="width: {Math.min(diskData.disk_usage, 90)}%"></div>
...
<style>
  ...
  @media (min-width: 900px) {
    .nd-grid { grid-template-columns: 280px 1fr; }
  }
  ...
  .chip-online {
    background: #1D2F20; border: 1px solid #1D2F20; color: #36B37E;
    box-shadow: 2px 2px 8px 0 rgba(27, 240, 27, 0.50);
  }
  .chip-offline {
    background: #331B1E; border: 1px solid #331B1E; color: #FF5630;
    box-shadow: 2px 2px 8px 0 rgba(240, 27, 62, 0.50);
  }
  .status-dot { width: 10px; height: 10px; border-radius: 50%; }
  .dot-online { background: #36B37E; }
  .dot-offline { background: #FF5630; }

  .node-id-row {
    display: flex; align-items: center; gap: 8px;
    margin: 12px 0 4px;
  }
  .node-id-text { font-size: 19px; font-weight: 500; color: white; }
  .node-sep { color: rgba(255,255,255,0.3); }
  .node-name-text { font-size: 14px; color: white; }
  .node-desc { font-size: 14px; color: white; margin-top: 4px; }
  ...
  .info-label { font-size: 14px; font-weight: 600; color: var(--text-secondary, #8892A4); width: 60%; }
  .info-value { font-size: 14px; color: var(--text-primary, #E0E4EB); }
  .info-badge {
    display: inline-block; padding: 2px 10px;
    border-radius: 16px; font-size: 12px; font-weight: 500;
    border: 1px solid;
  }
  .badge-ok { color: #36B37E; border-color: #36B37E; background: transparent; }
  .badge-fail { color: #FF5630; border-color: #FF5630; background: transparent; }
  ...
  .disk-label { font-size: 17px; font-weight: 500; color: #AFB7C8; }
  .disk-pct { font-size: 28px; font-weight: 500; color: var(--text-primary, #E0E4EB); }
  .disk-used { font-size: 16px; color: #D1D6E0; margin-bottom: 8px; }
  .disk-used-bold { font-weight: 500; color: var(--text-primary, #E0E4EB); }
  .progress-track {
  ...
  .progress-fill {
    ...
    height: 100%; background: #4A3BFF; border-radius: 5px;
  ...
  @media (min-width: 900px) {
    .chart-cell { height: calc((100vh - 240px) / 2); }
  }
  ...
  .tab-btn:not(:last-child) { border-right: 1px solid var(--border-color, #2E3649); }
  .tab-btn.tab-active {
