...
<div class="card" style="padding:0;overflow:auto;">
  ...
  {:else if items.length === 0}
    <div style="padding:40px;text-align:center;color:var(--text-muted)">No identifiers found</div>
  {:else}
    <table>
      <thead><tr>
      ...
      <tbody>
        {#each items as item, i}
          <tr class="clickable" on:click={() => goto(`/dashboard/nodes/${nodeId}/identify/${item.name}`)}>
