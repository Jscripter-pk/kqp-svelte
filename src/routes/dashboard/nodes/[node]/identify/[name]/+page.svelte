<script lang="ts">
  ...
  async function loadData() {
    loading = true;
    try {
      const res = await fetcher(endpoints.identify.detail(nodeId, name)) as any;
      item = res?.data ?? null;
...
<Breadcrumb node={nodeId} pages={[
  { pageName: 'Identifier List', href: `/dashboard/nodes/${nodeId}/identify` },
  { pageName: name }
]} />
<h1 class="page-title">Identifier: {name}</h1>

{#if loading}
  <div class="card" style="display:flex;justify-content:center;padding:60px"><span class="loading-spinner"></span></div>
{:else if !item}
  <div class="card" style="padding:40px;text-align:center;color:var(--text-muted)">Identifier not found</div>
{:else}
