<script lang="ts">
  ...
  async function loadData() {
    loading = true;
    try {
      const res = await fetcher(endpoints.spec.detail(nodeId, name)) as any;
      item = res?.data ?? null;
...
<Breadcrumb node={nodeId} pages={[
  { pageName: 'SPEC List', href: `/dashboard/nodes/${nodeId}/spec` },
  { pageName: name }
]} />
...
{:else if !item}
  <div class="card" style="padding:40px;text-align:center;color:var(--text-muted)">SPEC not found</div>
{:else}
  <div class="detail-grid">
    <div class="card detail-card">
      ...
      <div class="field-list">
        ...
        <div class="field-row"><span class="field-label">Version</span><span class="field-value">{item.version ?? '—'}</span></div>
      ...
      <table>
        ...
        <tbody>
          {#each item.fields as f}
            <tr><td>{f.name}</td><td>{f.type}</td><td>{f.size ?? '—'}</td><td>{f.desc ?? '—'}</td></tr>
          {/each}
        </tbody>
      </table>
    </div>
