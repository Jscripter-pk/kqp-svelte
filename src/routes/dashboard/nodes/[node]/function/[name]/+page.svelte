<script lang="ts">
  ...
  async function loadData() {
    loading = true;
    try {
      const res = await fetcher(endpoints.function.detail(nodeId, name)) as any;
      item = res?.data ?? null;
...
<Breadcrumb node={nodeId} pages={[
  { pageName: 'Function List', href: `/dashboard/nodes/${nodeId}/function` },
  { pageName: name }
]} />
<h1 class="page-title">Function: {name}</h1>

{#if loading}
  <div class="card" style="display:flex;justify-content:center;padding:60px"><span class="loading-spinner"></span></div>
{:else if !item}
  <div class="card" style="padding:40px;text-align:center;color:var(--text-muted)">Function not found</div>
{:else}
  <div class="detail-grid">
    <div class="card detail-card">
      ...
      <div class="field-list">
        ...
        <div class="field-row"><span class="field-label">Type</span><span class="field-value">{item.type ?? '—'}</span></div>
      ...
      <table>
        ...
        <tbody>
          {#each item.params as p}
            <tr><td>{p.name}</td><td>{p.type}</td><td>{p.desc ?? '—'}</td></tr>
          {/each}
        </tbody>
      </table>
    </div>
