<script lang="ts">
  ...
  async function loadData() {
    loading = true;
    try {
      const res = await fetcher(endpoints.layouts.detail(nodeId, name)) as any;
      item = res?.data ?? null;
...
{:else if !item}
  <div class="card" style="padding:40px;text-align:center;color:var(--text-muted)">Layout not found</div>
{:else}
