<script lang="ts">
  ...
  async function loadData() {
    loading = true;
    try {
      const res = (await fetcher(endpoints.rules.detail(nodeId, name))) as any;
...
<Breadcrumb
  node={nodeId}
  pages={[
    { pageName: "Rule List", href: `/dashboard/nodes/${nodeId}/rules` },
    { pageName: name },
  ]}
/>
  ...
  <div class="detail-grid">
    <div class="card detail-card">
      ...
      <div class="field-list">
        ...
        <div class="field-row">
          <span class="field-label">Ref. Layout</span><span class="field-value"
            >{item.ref_layout ?? "—"}</span
          >
        </div>
        <div class="field-row">
          <span class="field-label">Ref. Process</span><span class="field-value"
            >{item.ref_process !== undefined
              ? Number(item.ref_process).toLocaleString()
              : "—"}</span
          >
        </div>
        <div class="field-row">
          <span class="field-label">Ref. Actions</span><span class="field-value"
            >{item.ref_actions ?? "—"}</span
          >
        </div>
        <div class="field-row">
