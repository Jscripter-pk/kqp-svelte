<script lang="ts">
  ...
  async function loadData() {
    loading = true;
    try {
      const res = await fetcher(endpoints.rules.list(nodeId)) as any;
      items = res?.data?.list ?? [];
    ...
    <table>
      ...
      <tbody>
        {#each items as item, i}
          <tr class="clickable" on:click={() => goto(`/dashboard/nodes/${nodeId}/rules/${item.name}`)}>
